...
请你仔细阅读DEPLOYMENT_GUIDE.md，检查一下这些服务是否都运行着，是的话请你帮我停止这些服务，然后逐个逐个的启动这些服务，最后启动memos这个项目服务。
请你仔细阅读这个memos项目，告诉我这个项目的核心功能是什么？
你能分别讲解一下这5个记忆模块分别在什么情况下工作？请你根据memos的代码，举一个实际的场景，来一步一步模拟讲解这5个记忆模块是怎么工作的？ 并在最后用文本线条绘制出全流程                                                                                                           
  - 工作记忆（Working Memory）：短期、活跃的记忆                                                                                                                        
  - 长期记忆（Long-Term Memory）：持久化存储的大量记忆                                                                     
  - 用户记忆（User Memory）：用户特定的偏好和设定                                                                                   
  - 激活记忆（Activation Memory）：KV Cache 加速                                                                               
  - 参数化记忆（Parametric Memory）：结构化参数存储

我想知道为什么要用Neo4j和Qdrant混用？全都用Neo4j不行吗？而且neo4j和普通sql在这种agent记忆管理机制中，有什么不同吗？
现在我想知道，长期记忆和用户记忆之间的机制不同？长期记忆你说存在一个三级的树状结构？这个树状结构是仅存在于长期记忆中还是其他记忆也存在？合并节点是什么时候发生的？存在定时器的概念吗？用户记忆和工作记忆是树状结构还是扁平的一个一个并列排序的状态？ 相似节点是怎么判断的？聚类是用来干嘛的？
